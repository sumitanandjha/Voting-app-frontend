...
//         )}
//         {voteError && (
//           <div className="mt-4 text-red-500 text-center">{voteError}</div>
//         )}
//         {voteSuccess && (
//           <div className="mt-4 text-green-500 text-center">{voteSuccess}</div>
//         )}
//       </div>
//     </div>
//   );
...
const ElectionDetailsPage = () => {
    ...
    useEffect(() => {
        const fetchElectionDetails = async () => {
            try {
                const response = await axios.get(`https://vote-app-backend.vercel.app/election/${id}`);
                const electionData = response.data;
                setElection(electionData);
    ...
    const handleVote = async () => {
        try {
            const response = await axios.post(`https://vote-app-backend.vercel.app/cast/${id}`, { party: selectedParty }, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
